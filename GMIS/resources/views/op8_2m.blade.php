<!DOCTYPE html>
<html>
<title>Search, view all groups in a class - GMIS</title>
<head>
...
<!-- Operation View-->
  ...
  <div class="row">
        ...
        <form method="GET" action="{{route('op8_1m')}}" id="searchClasses">
        <div class="row">
            
			<div class="col-10 p-3">
 				<input type="text" class="form-control" name="searchID" placeholder="Enter class ID">
        	</div>
   ...
   <div class="row">
        ...
        <!--Class list-->
		<div class="col-6 p-4 border border-2">
            <h1 class="display-5 fs-4">Class List</h1>
 			<table class="table text-center px-2">
  				...
  				<tbody style="height: 10px !important; overflow: scroll; ">
    				<tr>
      					<td>{{$gmis_class->day}}</td>
                    	<td>{{$gmis_class->start}}</td>
                    	<td>{{$gmis_class->end}}</td>
                    	<td>{{$gmis_class->room}}</td>
      					<td>
            				<form action="{{route('op8_2m',['id'=>$gmis_class->class_id])}}">
        ...
        <!--Group list-->    
		<div class="col-6 p-4 border border-2 border-start-0">
            <h1 class="display-5 fs-4">Group List</h1>
        	<table class="table text-center px-2">
  				<thead>
    				<tr class="text-center">
  				...
  				<tbody style="height: 10px !important; overflow: scroll; ">
    				@foreach ($gmis_studentGroup as $row)
    				<tr>
